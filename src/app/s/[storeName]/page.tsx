...
export default function StoreProductList({
  ...
  const initInputCategories = categoryIds ? 
    categoryOptions.filter(category => categoryIds?.includes(category.value))
    : 
    [];
  ...
  const [inputCategories, setInputCategories] = useState<ICategoryInput[]>(initInputCategories);
  ...
  useEffect(() => {
    ...
    const query = createQueryString(searchParams, { key: 'categories', value: categoryIds.join(',')});
